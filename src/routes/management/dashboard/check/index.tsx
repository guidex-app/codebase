...
const Check: FunctionalComponent<CheckProp> = ({ activity }: CheckProp) => {
  ...
  return (
    <Fragment>
      {checkData.isOnline ? (
        <Item type="success" editLabel="Status ändern" label="Ihr Erlebnis ist online" text="Vorschau ansehen / Offline stellen" action={() => setOpenModal(true)} />
      ) : (
        <Item type="success" editLabel="Status ändern" label={`${checkData.percent}% ${checkData.isReady ? 'Erledigt - Online stellen' : 'abgeschlossen - Die nächsten Schritte ansehen'}`} action={() => setOpenModal(true)} />
      )}
